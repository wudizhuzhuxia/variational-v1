...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("{}", "=".repeat(80));
    println!(">> SPOT MARKET SELL ORDER");
    println!("{}", "=".repeat(80));
    println!();
    ...
    // Spot market sell order - market_index must be u8 (0-255)
    ...
    let sell_order = CreateOrderRequest {
        account_index,
        order_book_index: 1,      // Spot market
    ...
    println!("Creating spot sell order...");
    ...
    match client.create_market_order(
        sell_order.order_book_index,       // u8 for order_book_index
        sell_order.client_order_index,
        sell_order.base_amount,
        sell_order.price,
        sell_order.is_ask,
    ).await {
        Ok(response) => {
            ...
            if code == 200 {
                println!("SUCCESS - Spot sell order placed!");
